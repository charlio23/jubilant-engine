    ...
    public List getSuccessors (Object o) {
        ...
        for (int id = 0; id < board.getClientesSize (); ++id) {
            ...
            for (; jd < board.getCentralesSize (); ++jd) {
                ...
                if (newBoard.setCentral (id, jd)) {
                    successors.add (new Successor (id + " + " + jd + " - " +(board.getState ())[id], newBoard));
                }
            }
        }
        ...
        for (int id = 0; id < board.getClientesSize (); ++id) {
            ...
            if (state[id] != -1 && !centralesSobreProd[state[id]]){
                try {
                    double extra = board.getConsumo (id) / (1 - board.perdida (id, state[id]));
                    if (suministro[state[id]] + extra >
                        board.getProduccion (state[id])){
                        newBoard.setCentral (id, -1);
                    }
                    else {
        ...
        for (int jd = 0; jd < board.getCentralesSize (); ++jd){
            if (suministro[jd] > 0){
                ...
                id--;
                newBoard.setCentral (id, -1);
                successors.add (new Successor ("~c =" + jd + " -> " + count, newBoard));
            }
        }
            ...
            if (!centralesEnMarcha[jd]) {
                ...
                while (maxDemanda > 0 && id < board.getClientesSize ()) {
                    if (clientesSinEnergia[id]){
                        newBoard.setCentral (id, jd);
                        count++;
                        maxDemanda -= board.getConsumo (id) / (1 - board.perdida (id, jd));
                        successors.add (new
